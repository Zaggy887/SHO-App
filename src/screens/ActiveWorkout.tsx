    ...
    <Sheet open={open} onClose={onClose} title={session.name} full>
      ...
      <div className="space-y-3">
        {session.exercises.map((ex, exIdx) => {
          ...
          return (
            <div key={ex.defId} className={`overflow-hidden rounded-2xl border border-l-[3px] bg-ink-800 transition ${exDone ? 'border-white/8 border-l-brand-400/70' : 'border-white/5 border-l-brand-400/30'} ${isOptional ? 'opacity-70' : ''}`}>
              {/* Header */}
              <div className="flex items-center gap-3 p-3.5">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-1.5 flex items-start gap-1.5 text-[12px] leading-snug text-white/55">
                    <Target size={12} className="mt-0.5 shrink-0 text-brand-400/60" />
                ...
                {ex.sets.map((set, i) => (
                  <span
                    ...
                    className={`rounded-lg px-2 py-1 text-[11px] font-bold tabular-nums ${
                      set.done ? 'bg-brand-400/[0.09] text-brand-300' : 'bg-white/[0.05] text-white/60'
                    }`}
                  >
              ...
              <div className="flex items-stretch gap-2 border-t border-white/5 p-2.5">
                ...
                <button onClick={() => startAt(exIdx)} className="flex flex-1 items-center justify-center gap-1.5 rounded-xl bg-brand-400/12 py-2.5 text-[12px] font-bold text-brand-300 active:bg-brand-400/20">
