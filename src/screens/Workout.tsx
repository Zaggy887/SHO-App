...
import { CalendarDays, Clock, Play, ChevronRight, Check, CheckSquare } from 'lucide-react'
            ...
            return (
              <div key={e.defId} className="flex items-center gap-3 rounded-2xl border border-white/5 bg-ink-800 p-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-[12px] text-white/50">{e.targetSets} sets • {e.targetReps} reps</p>
                </div>
                <Chip color={done ? 'green' : 'gray'}>{fmtWeight(topWeight, units, units === 'imperial' ? 0 : 1)}</Chip>
                <button onClick={() => nav.open('activeWorkout')}><ChevronRight size={18} className="text-white/30" /></button>
              </div>
            )
          })}
        </div>
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 gap-3">
        {filtered.map((e) => (
          <div key={e.id} className="overflow-hidden rounded-2xl border border-white/5 bg-ink-800">
